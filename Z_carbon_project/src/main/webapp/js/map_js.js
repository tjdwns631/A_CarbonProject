...
function toolDist() { //면적측정
	//clearUIMapTools();
	if (mouse_work == mouse_mode.mm_measure_dist) {
		mouse_work = mouse_mode.mm_move_grab;
		$('#map_tool_anal_area').removeClass("on");
		$('#map_tool_ppt_width').removeClass("on");
		drawClear();
	} else {
		mouse_work = mouse_mode.mm_measure_dist;
		$('#map_tool_anal_area').addClass("on");
		$('#map_tool_ppt_width').removeClass("on");
		setDraw('Polygon');
	}
}
function toolArea() {// 거리측정
	//clearUIMapTools();
	if (mouse_work == mouse_mode.mm_measure_area) {
		...
		$('#map_tool_ppt_width').addClass("on");
		$('#map_tool_anal_area').removeClass("on");
		setDraw('LineString');
